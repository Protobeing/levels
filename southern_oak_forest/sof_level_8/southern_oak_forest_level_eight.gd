...
#group_eight
var enemies_plus_six_eight: PackedScene = preload('res://enemies/groupings/group_eight/enemies_plus_six_eight.tscn')
var enemies_plus_four_eight: PackedScene = preload('res://enemies/groupings/group_eight/enemies_plus_four_eight.tscn')
var enemies_below_three_eight: PackedScene = preload('res://enemies/groupings/group_eight/enemies_below_three_eight.tscn')
...
func _ready():
	...
	Levels.escaping = false
			...
			#AudioPlayer.oak_forest_chill_vibes = true
			#AudioPlayer.play_song()
	PlayerData.death_in_sof_8 = false
	enemies_eight()
	Global.blue_flower_heal = true
	$Jed.position = Vector2(-328,575)
...
func _on_jed_died() -> void:
	PlayerData.death_in_sof_8 = true
...
func enemies_eight():
	if Global.chick_counter <= 3:
			var enemies = enemies_below_three_eight.instantiate()
			enemies.position = $".".position
			call_deferred('add_child', enemies)
		
	elif Global.chick_counter <= 7:
		if Global.chick_counter > 3:
			var enemies = enemies_plus_four_eight.instantiate()
			enemies.position = $".".position
			call_deferred('add_child', enemies)
		
	elif Global.chick_counter >= 8:
			var enemies = enemies_plus_six_eight.instantiate()
			enemies.position = $".".position
			call_deferred('add_child', enemies)
