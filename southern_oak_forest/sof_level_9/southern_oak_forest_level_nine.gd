...
func _ready():
	...
	PlayerData.death_in_sof_9 = false
	enemies_nine()
	Global.blue_flower_heal = true
	$Jed.position = Vector2(-364,-514)
...
func _on_jed_died() -> void:
	PlayerData.death_in_sof_9 = true
...
func enemies_nine():
	if Global.chick_counter <= 3:
			var enemies = enemies_below_three_nine.instantiate()
			enemies.position = $".".position
			call_deferred('add_child', enemies)
		
	elif Global.chick_counter <= 6:
		if Global.chick_counter > 3:
			var enemies = enemies_plus_four_nine.instantiate()
			enemies.position = $".".position
			call_deferred('add_child', enemies)
	...
	if Global.chick_counter >= 7:
			...
			var enemies = enemies_plus_six_nine.instantiate()
			enemies.position = $".".position
			call_deferred('add_child', enemies)
