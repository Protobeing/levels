...
#group_five
var enemies_plus_six_five: PackedScene = preload('res://enemies/groupings/group_five/enemies_plus_six_five.tscn')
var enemies_plus_four_five: PackedScene = preload('res://enemies/groupings/group_five/enemies_plus_four_five.tscn')
...
func _ready():
	...
	PlayerData.death_in_sof_5 = false
	if BlueFlowerCount.blue_flower_5:
		blue_flower_anim.play("no_heal")
	enemies_five()
	Global.blue_flower_heal = true
	$Jed.position = Vector2(311,239)
...
func _on_jed_died() -> void:
	PlayerData.death_in_sof_5 = true
...
func enemies_five():
	if Global.chick_counter <= 3:
			var enemies = enemies_below_three_five.instantiate()
			enemies.position = $".".position
			call_deferred('add_child', enemies)
		
	elif Global.chick_counter <= 6:
		if Global.chick_counter > 3:
			var enemies = enemies_plus_four_five.instantiate()
			enemies.position = $".".position
			call_deferred('add_child', enemies)
	...
	if Global.chick_counter >= 7:
			...
			var enemies = enemies_plus_six_five.instantiate()
			enemies.position = $".".position
			call_deferred('add_child', enemies)
