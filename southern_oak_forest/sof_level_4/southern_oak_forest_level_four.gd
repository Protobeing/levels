extends Node2D
#group_four
var enemies_plus_six_four: PackedScene = preload('res://enemies/groupings/group_four/enemies_plus_six_four.tscn')
var enemies_plus_four_four: PackedScene = preload('res://enemies/groupings/group_four/enemies_plus_four_four.tscn')
...
func _on_jed_died() -> void:
	PlayerData.death_in_sof_4 = true
...
func enemies_four():
	if Global.chick_counter <= 3:
			var enemies = enemies_below_three_four.instantiate()
			enemies.position = $".".position
			call_deferred('add_child', enemies)
		#
	elif Global.chick_counter <= 6:
		if Global.chick_counter > 3:
			var enemies = enemies_plus_four_four.instantiate()
			enemies.position = $".".position
			call_deferred('add_child', enemies)
			...
			var enemies = enemies_plus_six_four.instantiate()
			enemies.position = $".".position
			call_deferred('add_child', enemies)
